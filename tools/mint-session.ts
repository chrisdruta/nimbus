// Dev tool: mint a nimbus_session cookie for the owner so browser
...
if (!secretRaw || !ownerRaw) {
  console.error("SESSION_SECRET and OWNER_SC_USER_ID must be set (source .env first)");
  process.exit(1);
}

const owner = await getUserByScId(Number(ownerRaw));
if (!owner) {
  console.error(`No users row for OWNER_SC_USER_ID=${ownerRaw} — complete OAuth in production first`);
  process.exit(1);
}

const token = await new SignJWT({ userId: owner.id, scUserId: owner.sc_user_id })
